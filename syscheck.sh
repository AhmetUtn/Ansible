...
# Sistemin dağıtımını öğren (Ubuntu mu Debian mı?)
distro=$(lsb_release -is 2>/dev/null || cat /etc/os-release | grep '^ID=' | cut -d'=' -f2)

# Ubuntu veya Debian versiyonunu öğren
...
# needrestart ile yeniden başlatma gereksinimi kontrolü (otomatik mod ve sessiz mod)
...
if ! command -v needrestart &> /dev/null; then
    ...
    #kernel_check=$(sudo needrestart -k -b -p)
...
while IFS= read -r line; do
    line=$(echo "$line" ) #  | sed 's/"/\\"/g')  # Çift tırnakları kaçış yap
...
# Güncellemelerin durumu ve detaylı öneriler
...
if [ "$updates" -gt 0 ]; then
    recommendations+=("recommendation,medium,maintenance,\"$updates adet paket güncellemeye hazır. 'sudo apt update && sudo apt upgrade' komutu ile güncellemeleri yapın.\"")
fi

if [ ! -z "$security_updates_list" ]; then
    recommendations+=("recommendation,high,security,\"Güvenlik güncellemeleri mevcut! 'sudo apt update && sudo apt upgrade' ile güvenlik güncellemelerini yapın.\"")
fi
...
security_updates=$(apt list --upgradable 2>/dev/null | grep "security" | wc -l || echo "Security updates check failed")
